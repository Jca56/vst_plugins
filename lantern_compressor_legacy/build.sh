#!/usr/bin/env bash
# Cross-compile Lantern Compressor into a Windows VST3 (+ CLAP) bundle, so
# Ableton Live running under Wine can load it.
set -euo pipefail
...
echo ">> Building Lantern Compressor for $TARGET (release)..."
cargo xtask bundle lantern_compressor --release --target "$TARGET"
